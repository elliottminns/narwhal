...
class HTTPParser {
  ...
  func parse() throws -> HTTPResponse {
    ...
    return HTTPResponse(status: statusCode, headers: headers, body: body)
  }

  func parseHeaders(headers: [String]) -> [String: String] {
    
    let headerPairs: [(String, String)] = headers.map { $0.split(":", maxSplit: 1) }.flatMap {
      if $0.count == 2 {
